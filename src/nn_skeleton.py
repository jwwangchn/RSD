...
class ModelSkeleton:
    ...
    def __init__(self, mc):
        ...
        # image batch input NHWC structure
        ...
        # box regression parameter
        # TODO: add angle
        self.ph_box_delta_input = tf.placeholder(
            tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, 5], name='box_delta_input')
        # Tensor used to represent bounding box coordinates.
        # groundtruth bboxes
        # TODO: add angle
        self.ph_box_input = tf.placeholder(
            tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, 5], name='box_input')
    ...
    def _add_interpretation_graph(self):
        ...
        with tf.variable_scope('interpret_output') as scope:
            ...
            self.pred_class_probs = tf.reshape(tf.nn.softmax(tf.reshape(preds[:, :, :, :num_class_probs], [-1, mc.CLASSES])), 
                                                [mc.BATCH_SIZE, mc.ANCHORS, mc.CLASSES], name='pred_class_probs')
        ...
        with tf.variable_scope('bbox') as scope:
            with tf.variable_scope('stretching'):
                delta_x, delta_y, delta_w, delta_h, delta_angle = tf.unstack(self.pred_box_delta, axis=2)

                anchor_x = mc.ANCHOR_BOX[:, 0]
                anchor_y = mc.ANCHOR_BOX[:, 1]
                anchor_w = mc.ANCHOR_BOX[:, 2]
                anchor_h = mc.ANCHOR_BOX[:, 3]
                anchor_angle = mc.ANCHOR_BOX[:, 4]

                box_center_x = tf.identity(anchor_x + delta_x * anchor_w, name='bbox_cx')
                box_center_y = tf.identity(anchor_y + delta_y * anchor_h, name='bbox_cy')
                box_width = tf.identity(anchor_w * util.safe_exp(delta_w, mc.EXP_THRESH), name='bbox_width')
                box_height = tf.identity(anchor_h * util.safe_exp(delta_h, mc.EXP_THRESH), name='bbox_height')
                box_angle = tf.identity(180.0 * delta_angle / np.pi + anchor_angle, name="box_angle")
                ...
                self._activation_summary(delta_angle, 'delta_angle')
        ...
        with tf.variable_scope('IOU'):
            def _tensor_iou(box1, box2):
                # calculator iou between box1 and box2
    ...
    # TODO: to (xmin, ymin, xmax, ymax, theta)
    # finally output
