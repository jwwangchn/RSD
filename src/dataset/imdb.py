...
from utils.util import get_iou_matrix_tf
...
class imdb(object):
    ...
    def _shuffle_image_idx(self):
        self._perm_idx = [self._image_idx[i] for i in np.random.permutation(np.arange(len(self._image_idx)))]
        self._cur_idx = 0
    ...
    def read_batch(self, shuffle=True):
        ...
        for idx in batch_idx:
            ...
            # load annotations
            label_per_batch.append([b[5] for b in self._rois[idx][:]])
            gt_bbox = np.array([[b[0], b[1], b[2], b[3], b[4]] for b in self._rois[idx][:]])    # (cx, cy, w, h, angle)
            ...
            if mc.DATA_AUGMENTATION:
                ...
                if np.random.randint(2) > 0.5 and mc.DATA_AUGMENTATION:
                    im = im[:, ::-1, :]
            ...
            for i in range(len(gt_bbox)):
                overlaps = get_iou_matrix_tf(mc.ANCHOR_BOX, gt_bbox[i])
                ...
                box_cx, box_cy, box_w, box_h, box_angle = gt_bbox[i]
                delta = [0] * 5
                ...
                delta[4] = (box_angle - mc.ANCHOR_BOX[aidx][4]) * np.pi / 180.0
